...
namespace charts
{
    ...
    /// Uses parameterized queries and dynamic country filtering for compatibility with older SQL Server versions.
    /// </summary>
    public class ChartDataHandler : IHttpHandler
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                {
                    // Determine grouping column safely
                    string groupCol;
                    ...
                    if (hasCountries)
                    {
                        // Build parameter names for countries
...
SELECT
    ...
    COUNT(*) AS Value,
    STUFF((
        SELECT DISTINCT ';' + t2.FilePath
        FROM SectionsITR t2
        WHERE t2.[{groupCol}] = t1.[{groupCol}]
          AND {whereT2}
        FOR XML PATH(''), TYPE
    ).value('.', 'NVARCHAR(MAX)'), 1, 1, '') AS FileUrls
FROM SectionsITR t1
WHERE {whereT1}
...
ORDER BY t1.Country ASC, t1.LawYear ASC;";
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            string fileUrls = reader["FileUrls"] as string ?? string.Empty;
                            ...
                            var distinctUrls = fileUrls
                                .Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries)
                                .Select(u => u.Replace(@"C:\ITR_solution", "").Replace("\\", "/"))
                                .Distinct()
                                .ToList();
                            ...
                            points.Add(new ChartPoint
                            {
                                ...
                                Value = val,
                                Tooltip = tooltip,
                                FileUrls = distinctUrls
                            });
                        }
                    }
                }

                // 3) Serialize back as JSON and return
                var json = new JavaScriptSerializer().Serialize(points);
                context.Response.Write(json);
            }
            catch (Exception ex)
            {
                // TODO: integrate your logging here
                context.Response.StatusCode = 500;
